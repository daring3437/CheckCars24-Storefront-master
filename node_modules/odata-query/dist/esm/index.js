...
export var alias = function (name, value) { return ({
    type: 'alias', name: name, value: value,
    handleName: function () {
        return "@" + this.name;
    },
...
export default function (_a) {
    var _b = _a === void 0 ? {} : _a, $select = _b.select, $search = _b.search, $top = _b.top, $skip = _b.skip, $skiptoken = _b.skiptoken, $format = _b.format, filter = _b.filter, transform = _b.transform, orderBy = _b.orderBy, key = _b.key, count = _b.count, expand = _b.expand, action = _b.action, func = _b.func, aliases = _b.aliases;
    var path = '';
    var params = {
    ...
    if (func) {
        if (typeof func === 'string') {
            ...
            var funcName_1 = Object.keys(func)[0];
            var funcArgs = Object.keys(func[funcName_1]).reduce(function (acc, item) { return acc.concat([item + "=" + handleValue(func[funcName_1][item])]); }, []);
            path += "/" + funcName_1;
            if (funcArgs.length) {
                path += "(" + funcArgs.join(',') + ")";
            }
    ...
    if (aliases) {
        aliases
            ...
            var _a;
            return Object.assign(acc, (_a = {}, _a[alias.handleName()] = alias.handleValue(), _a));
        }, params);
    }
    return buildUrl(path, tslib_1.__assign({ $select: $select, $search: $search, $top: $top, $skip: $skip, $skiptoken: $skiptoken, $format: $format }, params));
}
function buildFilter(filters, propPrefix) {
    ...
    return (Array.isArray(filters) ? filters : [filters])
        ...
        if (filter) {
            var builtFilter = buildFilterCore(filter, propPrefix);
            if (builtFilter) {
    ...
    function buildFilterCore(filter, propPrefix) {
            ...
            var filtersArray = Object.keys(filter).reduce(function (result, filterKey) {
                ...
                if (['number', 'string', 'boolean'].indexOf(typeof value) !== -1 ||
                    ...
                    var builtFilters = value
                        .map(function (v) { return buildFilter(v, propPrefix); })
                            ...
                            if (COMPARISON_OPERATORS.indexOf(op) !== -1) {
                                ...
                                if (Array.isArray(value[op])) {
                                    result.push(value[op]
                                        .map(function (v) { return '(' + buildFilterCore(v, propName) + ')'; })
                                        .join(" " + op + " "));
                                }
                                else {
                                ...
                                result.push(propName + ' in (' + resultingValues.map(function (v) { return handleValue(v); }).join(',') + ')');
                            }
                                ...
                                // Nested property
                                var filter_1 = buildFilterCore(value, propName);
                                if (filter_1) {
    ...
    function buildCollectionClause(lambdaParameter, value, op, propName) {
        ...
        if (value) {
            ...
            var filter = buildFilterCore(Array.isArray(value)
                ? value.reduce(function (acc, item) { return (tslib_1.__assign({}, acc, item)); }, {})
...
function handleValue(value) {
    if (typeof value === 'string') {
        ...
        var arr = value.map(function (d) { return (typeof d === 'string' ? "'" + d + "'" : d); });
        return "[" + arr.join(',') + "]";
    }
    else {
